...
function main() {
  ...
  // Merge folder data if available. A corrupt folders-data.json (e.g. from
  // an interrupted earlier run) must not crash the export — treat it as empty.
  ...
  if (fs.existsSync(FOLDERS_PATH)) {
    try {
      ...
      console.warn("Ignoring corrupt folders-data.json:", e.message);
    }
    let tagged = 0;
