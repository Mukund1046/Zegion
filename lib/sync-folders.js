    ...
    if (!response.ok) {
      ...
      const hint =
        response.status === 401 || response.status === 403
          ...
          : "";
      throw new Error(
        `X returned ${response.status} ${response.statusText} for ${operation}: ${errorBody.slice(0, 300)}${hint}`
      );
    }
...
// --- Fetch helpers: retry + bounded concurrency ---

function isRateLimitError(error) {
  const message = error instanceof Error ? error.message : String(error);
  return /X returned 429/.test(message);
}
...
// Retry only on X rate limiting (429) with capped exponential backoff.
// Auth failures (401/403) and other errors fail fast so bad credentials
// surface immediately instead of hanging through retries. `fetchImpl` is an
// injection point for regression tests; production always uses fetchGraphQL.
async function fetchGraphQLWithRetry(queryId, operation, variables, maxAttempts = 4, fetchImpl = fetchGraphQL) {
  let lastError;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      return await fetchImpl(queryId, operation, variables);
    } catch (error) {
      lastError = error;
...
// Run async work over items with at most `limit` in flight so X rate limits
// are not tripped by unbounded Promise.all fan-out. Results keep input order.
...
// Safety cap so a stuck cursor can never loop forever.
const MAX_FOLDER_PAGES = 100;
const FOLDER_CONCURRENCY = 3;

// --- Fetch all pages of a folder ---

async function fetchFolderBookmarks(folderId, folderName, fetchFn = fetchGraphQLWithRetry) {
  const allTweets = [];
  let cursor = null;
  let page = 0;
  ...
  while (true) {
    ...
    if (page > MAX_FOLDER_PAGES) {
      console.error(
        `\n  Stopping ${folderName} after ${MAX_FOLDER_PAGES} pages (pagination guard)`
      );
      break;
    }
    const variables = {
    ...
    let json;
    try {
      json = await fetchFn("LML09uXDwh87F1zd7pbf2w", "BookmarkFolderTimeline", variables);
    } catch (e) {
...
async function main() {
  ...
  // 2. Fetch bookmarks for each folder (bounded parallelism)
  ...
  const folderResults = await mapWithConcurrency(
    ...
    FOLDER_CONCURRENCY,
    (folder) =>
      fetchFolderBookmarks(folder.id, folder.name).then((tweets) => ({ folder, tweets }))
  );

  for (const { folder, tweets } of folderResults) {
    for (const t of tweets) {
      if (!folderMap[t.tweetId]) folderMap[t.tweetId] = new Set();
      folderMap[t.tweetId].add(folder.name);
    }
  }

  // Convert Sets to arrays for JSON output
  for (const tweetId of Object.keys(folderMap)) {
    folderMap[tweetId] = [...folderMap[tweetId]];
  }
