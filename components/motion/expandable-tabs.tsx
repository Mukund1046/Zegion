...
export interface ExpandableTabsProps {
  ...
  /** DialKit-tunable layout */
  barGap?: number;
  activeLeftPad?: number;
  activeRightPad?: number;
  labelGap?: number;
  panelGap?: number;
  /** When true, tabs do not expand to show label — only panel switches */
...
const TAB_W = 32;
const BAR_X = 16;
const BAR_GAP_DEFAULT = 4;
const ROOT_BORDER = 2;
const ICON_W = 16;
const ACTIVE_LEFT_PAD_DEFAULT = 10;
const ACTIVE_RIGHT_PAD_DEFAULT = 16;
const LABEL_GAP_DEFAULT = 7;
...
export function ExpandableTabs({
  ...
  className,
  classNames,
  barGap = BAR_GAP_DEFAULT,
  activeLeftPad = ACTIVE_LEFT_PAD_DEFAULT,
  activeRightPad = ACTIVE_RIGHT_PAD_DEFAULT,
  labelGap = LABEL_GAP_DEFAULT,
  panelGap = PANEL_DOCK_GAP,
  disableExpand = false,
}: ExpandableTabsProps & {
  barGap?: number;
  activeLeftPad?: number;
  ...
  const closedSize = {
    width:
      items.length * TAB_W +
      Math.max(0, items.length - 1) * barGap +
      BAR_X +
      ROOT_BORDER,
    ...
    (item: ExpandableTabsItem) => {
      ...
      return Math.max(
        ...
        activeLeftPad +
          ...
          activeRightPad +
          2, // small buffer to prevent clipping due to subpixel rounding
      );
    },
    [labelWidths, activeLeftPad, activeRightPad, labelGap, disableExpand],
  );
    ...
    <>
      ...
      >
        ...
        <div
          ...
          className={cn(
            "absolute bottom-0 left-1/2 z-20 flex -translate-x-1/2 items-center p-2",
            classNames?.bar,
          )}
          style={{ height: BAR_H, gap: barGap, maxWidth: "calc(100% - 16px)" }}
        >
          {items.map((item) => {
            ...
            return (
              <motion.button
                ...
                animate={{
                  width: disableExpand ? TAB_W : active && isActive ? activeTabWidth : TAB_W,
                }}
                transition={reduce ? { duration: 0 } : TAB_CHANGE_SPRING}
                style={
                  !disableExpand && active && isActive
                    ? { paddingLeft: activeLeftPad, paddingRight: activeRightPad }
                    : undefined
                }
                className={cn(
                  ...
                  !disableExpand && active && isActive && "min-w-0 justify-start",
                  isActive
                    ? "text-foreground"
              ...
              >
                ...
                <motion.span
                  ...
                  animate={
                    disableExpand
                      ? { width: 0, opacity: 0, marginLeft: 0, filter: "blur(0px)" }
                      : reduce
                        ? {
                            width: isActive ? labelWidth : 0,
                            opacity: isActive ? 1 : 0,
                            marginLeft: isActive ? labelGap : 0,
                            filter: "blur(0px)",
                          }
                        : {
                            width: isActive ? labelWidth : 0,
